...
public:
  ...
  exportable_html_element &operator<<(INJECTABLE &injectable) {
...
class html : public exportable_html_element {
public:
...
template <typename... children> class part : public exportable_html_element {
public:
  part(const std::map<std::string, std::string> &fields = {}) {
...
#define CREATE_BODY_BLOCK_COMPONENT_EXTENDED(NAME, EXPR)                       \
  template <typename... children> class NAME : body_element {                  \
  public:                                                                      \
    std::string &content;                                                      \
    NAME(std::string &content) : content(content) {                            \
      block_tag it(EXPR, content);                                             \
      (children(content), ...);                                                \
    }                                                                          \
  };
...
///! Same as div, created for consistency (avoiding hyper::div because of
///cstdlib's div function)
CREATE_BODY_BLOCK_COMPONENT_EXTENDED(block, "div");
///! Styles and Semantics: Defines a section in a document
...
///< select>, <textarea>	Specifies the name of the form the element belongs
///< to
...
///! quoted string
inline const std::string qs(std::string uqs) { return "\"" + uqs + "\""; }

class css_error : std::runtime_error {
public:
  css_error(std::string desc, std::string hint)
      : std::runtime_error("css-error: " + desc + ". hint: " + hint) {}
};
...
public:
  ...
  inline builder &operator()(const std::string &selector) {
    ...
    //     throw css_error("Selector \"" + selector + "\" was already
    //     specified", "try specifying another selector.");
    // }
  ...
  inline builder &operator<<(const std::pair<std::string, std::string> &prop) {
    ...
    if (not context_data) {
      throw css_error(
          "No selector specified, use css(<selector>) before << <css-prop>.",
          "try specifying some selector, eg: css(\"*\")");
    }
...
public:
  ...
  $on(std::string &content) : content(content) {
    content.insert(content.rfind(">"), std::string(" hx-on-" + value + "=\"") +
                                           value.c_str() + "\"");
  }
};
...
#define DEFINE_HTMX_ATTRIBUTE(NAME, ATTRIBUTE)                                 \
  ...
  public:                                                                      \
    ...
    std::map<std::string, std::string> pairs;                                  \
    $##NAME(std::string &content) : content(content) {                         \
      content.insert(content.rfind(">"), std::string(" ") + ATTRIBUTE +        \
                                             " =\"" + value.c_str() + "\"");   \
    }                                                                          \
  };
...
///! adds the disabled attribute to the specified elements while a request is in
///flight
